...
if {[catch {eval exec grep "cvw_t" $outputDir/hdl/$::env(DESIGN).sv}] == 0} {
	...
	set wrapper 1
    # make the wrapper
	exec python3.12 $::env(WALLY)/synthDC/scripts/wrapperGen.py $::env(DESIGN) $outputDir/hdl
}
echo "Done with the wrapper..."
...
# Verilog files
set my_verilog_files [glob $outputDir/hdl/cvw.sv $outputDir/hdl/*.sv $outputDir/hdl/*.svh ]
...
if { $maxopt == 1 } {
    ...
    # compile -no_seq_output_inversion -no_boundary_optimization
    compile -exact_map
}
